...
//	t_stamp_vec SingleA, // list of possible timestamp events
//				SingleC; // (combination of events from all the data sources)
...
//***********************For Debugging****************************************************
/*	cout<<"Freq: "<<min_freq<<endl<<"Conf: "<<min_conf<<endl<<"AntWidth: "<<wina<<endl<<
		"ConsWidth: "<<winc<<endl<<"LagWidth: "<<lag<<endl<<"Type: "<<type<<endl<<
		"Ants: "<<ants<<endl<<"Cons: "<<cons<<endl<<"LagType: "<<lagtype<<endl;
	for(i=0;i<eSetLen;i++){
		cout<<"Eventset: "<<eSet[i]<<endl;
	}
*/
//****************************************************************************************
...
/*//************For debugging -- remove****************************
		cout<<"FVecSize: "<<TimeFvec.size()<<endl;
		for(int l=0;l<TimeFvec.size();l++){
			for(int r=0;r<TimeFvec[l].size();r++){
				cout<<TimeFvec[l][r].getEvent()<<" ";
			}
			cout<<endl;
		}
*///****************************************************************
...
*/	//****************************************************************
	...
	switch(type)
	{
		...
		case 'q':
			...
			Generate_Parallel_CTMINEPI(eList, min_freq, k, wina, winc, A, C, type, epCntsXML);
		...
		case 't':
			...
			Generate_Serial_CTMINEPI(eList, min_freq, k, wina, winc, A, C, type, eSetLen, TimeFvec, epCntsXML);
...
void XMLIntFace::Generate_Parallel_CTMINEPI(string eList, int min_fr, int& k, int wina, int winc, const event_set& A, const event_set& C, char type, string &epCntsXML)
{
	int Candidates=0;
...
void XMLIntFace::Generate_MOWCATL_RULES(double min_conf, int maxk, int lag, int min_fr, char type, const event_set& T, const event_set& C,int ans, char lag_type, string& ruleListXML, string& ruleCntsXML)
{
...
void XMLIntFace::Generate_Serial_CTMINEPI(string eList, int min_fr, int& k, int wina, int winc, const event_set& A, const event_set& C, char type, int sources, const event_vec_vec& TimeFvec, string &epCntsXML)
{
