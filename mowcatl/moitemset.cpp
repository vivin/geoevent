...
int moitemset::indexoftimeA(const time_stamp& A, const t_stamp_vec& TimeF, int size)
{
	int i = 0;
	bool found = false;
	int save=size;
...
void moitemset::addtimestampitems(const event_vec_vec& TimeFvec, const event_set& B, int sources,int win)
			...
			while (more_moves_for_current_pawn)
			{
				...
				while (more_itr_settings)
				{
					...
					do
					{
						...
						if (next >=0)
							iters[next]++;
					} while ( next>=0 && iters[next] == TimeFvec[index[next]].end());
					if (next<0 )
...
//		system("pause");
	...
	int k = 2;
	for (const_moITEM_ITR itemitr = is.begin(); itemitr != is.end(); ++itemitr)
	{//MUST BE PROCCESSED IN ORDER OF k - all episodes of length 2, then 3, etc.
		//testing
		//(*itemitr)->output(cout);
//		system("pause");
...
void moitemset::addfreq(moitemset& fcc_k_b, moitemset& garbage, int min_fr)
{
	...
	for (moITEM_ITR item_set_itr = fcc_k_b.is.begin(); item_set_itr != fcc_k_b.is.end(); ++item_set_itr)
...
{
	for (moITEM_ITR item_set_itr = is.begin(); item_set_itr != is.end(); ++item_set_itr)
	// find all episodes that are frequent
	{
		//testing
//	(*item_set_itr)->output(cout);
...
void moitemset::fast_generate_parallel_candidates(moitemset& f_l, char type, int l)
{// Algorithm 3 from Mannila, Toivonen and Verkamo, "Discovery of frequent episodes 
...
//serial items have their events in serial order 
	...
	for (moITEM_ITR i_itr = f_l.is.begin(); i_itr != f_l.is.end(); ++i_itr)
	{
		...
		while ( (j_itr != f_l.is.end()) && ((*j_itr)->blockstart == (*i_itr)->blockstart) )
		{
...
void moitemset::fast_generate_serial_candidates(moitemset& f_l, char type, int l)
{// Algorithm 3 from Mannila, Toivonen and Verkamo, "Discovery of frequent episodes 
	// in event sequences" Department of Computer Science Series of Publications C,
	// report C-1997-15, University of Helsinki, Finland
	//with added closures
...
void moitemset::generate_parallel_candidates(const moitemset& fc_k_b, char type, const event_set& B, int k)
{
	//Step 1. create the candidate episodes - one at a time
	for (const_moITEM_ITR item_set_itr = fc_k_b.is.begin(); item_set_itr != fc_k_b.is.end(); ++item_set_itr)
	{
		for(event_set::const_iterator event_itr = B.begin(); event_itr != B.end(); ++event_itr)
		{


			bool needtoadd=true;
			moitem * cand= (*item_set_itr)->generate_parallel_candidate(*event_itr);
...
void moitemset::generate_parallel_rule_candidates(const moitemset& A, const moitem* Z, char type,  const event_set& B, int j)
...
void moitemset::fast_generate_parallel_rule_candidates(moitemset& f_l, const moitem* Z, char type, int l)
{// Algorithm 3 from Mannila, Toivonen and Verkamo, "Discovery of frequent episodes 
	// in event sequences" Department of Computer Science Series of Publications C,
	// report C-1997-15, University of Helsinki, Finland
...
void moitemset::generate_serial_candidates(const moitemset& fc_k_b, char type, const moitemset& B1, int k)
{
	//Step 1. create the candidate episodes - one at a time
	for (const_moITEM_ITR item_set_itr = fc_k_b.is.begin(); item_set_itr != fc_k_b.is.end(); ++item_set_itr)
	{
		for(const_moITEM_ITR event_itr = B1.is.begin(); event_itr != B1.is.end(); ++event_itr)
		{
			bool needtoadd=true;
			moitem * cand= (*item_set_itr)->generate_serial_candidate(*event_itr);
...
void moitemset::generate_serial_rule_candidates(const moitemset& A, const moitem* Z, char type, moitemset B1, int j)
	//generate the subsets of moitem Z, that meet constraints B
	//used only from the morule-generation process.
	//CHANGE when constraints are converted to DNF
{	//Step 1. create the candidate episodes - one at a time
...
void moitemset::fast_generate_serial_rule_candidates(moitemset& f_l, const moitem* Z, char type, int l)
{// Algorithm 3 from Mannila, Toivonen and Verkamo, "Discovery of frequent episodes 
	// in event sequences" Department of Computer Science Series of Publications C,
	// report C-1997-15, University of Helsinki, Finland
...
bool moitemset::does_contain_parallel(const moitem* Y) const
{ //parallel
...
bool moitemset::does_contain_parallel_rule_item(const moitem* Y) const
{ //parallel rule itme
...
bool moitemset::does_contain_serial_rule_item(const moitem* Y) const
{ //parallel rule itme
...
bool moitemset::does_contain_serial(const moitem* Y) const
{ //serial
...
bool moitemset::support_subs_parallel(const moitem* Y, const moitemset& fc_k_b, int k) const
{
...
bool moitemset::support_subs_serial(const moitem* Y, const moitemset& fc_k_b, int k) const
{
 int i,j;
	bool support = true;
	vector<int> index;
...
moitem* moitemset::findsmallest(const fc_type& split_fac, const moitem* X) const
{
...
//	cout<<"in items: ";
	...
	for (movej = 1; movej <= size;movej++)
		j++;
	for(movej=1;movej<X->getconsnumberofevents();movej++); //move past the same size antecedent, but smaller consequents
		j++;

	while (!item_found && j != split_fac.end())
	{//look through each level j

		i = j->is.begin(); 
		while (!item_found && i != j->is.end())
		{//look through each moitem i at the current level j
			//testing:
...
double moitemset::computelevelsupport(const moitem* Z) const
{//level support for s and t serial episodes
...
//					system("pause");
					...
					{
						morule r;
							r.insert((*itr)->get_antepisode(), double(X_antsupport), (*itr)->get_consepisode(), double(i_freq), (*itr)->get_last_occurrence(), (*itr)->get_conssupport());
//						r.output(cout);  //for testing
//						system("pause");
						if (!r.cons_empty() && !REAR.contain(r))
							REAR.insertrule(r);
...
void moitemset::loop1serial(const fc_type& split_fac, double c, mos_ruleset& REAR, int i, int k, char type, const event_set& T, const event_set& C)
{
	...
	//	(*itr)->output_episode(cout);
		...
		if ((*itr)->get_support() != maxsupport)
		{
					...
					{
						mos_rule r;
							r.insert((*itr)->get_antepisode(1), double(X_antsupport), (*itr)->get_consepisode(1), double(i_freq), (*itr)->get_last_occurrence(), (*itr)->get_conssupport());
						//r.output(cout);  //for testing
						if (!r.cons_empty() && !REAR.contain(r))
							REAR.insertrule(r);
					} //end if
...
void moitemset::combine(const moitemset& fa, moitemset& garbage, const fc_type& fc, char type, int lag, int min_fr, char lag_type)
...
//combine the ant and cons into ruleitems
{
 ...
 // 	moitem*  tryit; 
	...
	for (const_moITEM_ITR faitr = fa.is.begin(); faitr != fa.is.end(); ++faitr)
	{
		...
		while ( !done )		
		{	
			for (const_moITEM_ITR fcitr = fc[i].is.begin();fcitr != fc[i].is.end(); ++fcitr)
			{
