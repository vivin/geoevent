...
void morule::insert(const event_set& a, double a_support, const event_set& c, double rule_support, const occur last, double c_support, double s_density)
{
...
//type - The type of episode that the algorithm was looking for
{
	...
	//Consequent Support
	ruleXML += "<DensityRatio>";
	convertedNum = convertfcvt(density);
	StringHelper::trim(convertedNum," \0");
	ruleXML += convertedNum;
	ruleXML += "</DensityRatio>";
